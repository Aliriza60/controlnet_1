...
info_text = f"Script Version : 1.1 \
				...
				--seed: {ap.seed} \
				--single_controlnet_model_file: {ap.single_controlnet_model_file} \
...
if ap.single_controlnet_model_file == False:

	for con_type_1 in (ap.controlnet_type):

		if con_type_1 == "canny":

			controlnet.append(ControlNetModel.from_pretrained(ap.controlnet_model_paths, torch_dtype=torch.float16))
	...
	for con_type_2 in (ap.controlnet_type):
		...
		if con_type_2 == "canny":
			...
			controlnet_single = ControlNetModel.from_pretrained(ap.controlnet_model_paths, torch_dtype=torch.float16)
...
if ap.single_controlnet_model_file == False:
	...
	pipeline = StableDiffusionControlNetPipeline.from_single_file(ap.base_model_path, controlnet = controlnet_single, safety_checker=None)
